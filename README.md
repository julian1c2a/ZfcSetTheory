...
[![Modules](https://img.shields.io/badge/módulos-95-blue)](REFERENCE.md)
...
> **Estado actual** (2026-05-26): 95 archivos `.lean` bajo `ZFC/` (incl. barrels) · **0 sorry** · 0 errores · 0 warnings (`lake build`: 115 jobs).
> Phases 1–7 completas. **Phase 8 (Monomios y Polinomios)** en progreso: `Rat/Pow.lean` (exponenciación racional $x^n$) y `Algebra/Monomial.lean` (monomios sobre ℚ, grado WithBot ω) completos; siguiente `Algebra/Polynomial.lean`.
...
**Lo que hace el proyecto ahora** (2026-05-26):
...
| `Rat/TupleSeq.lean` | seqSumQ, seqProdQ (sumatorio/productorio de tuplas racionales) vía RecursionTheoremWithStep |

### Phase 8: Monomios y Polinomios (🔶 En progreso)

| Módulo | Exports clave | Estado |
|--------|---------------|--------|
| `Rat/Pow.lean` | powRatQ ($x^n$), powRatQ_zero/succ/mem_RatSet/one, powRatQ_zero_base, powRatQ_one_base, powRatQ_add_exp ($x^{n+m}=x^n x^m$), powRatQ_mul_base ($(xy)^n=x^n y^n$) | ✅ Completo (prerequisito) |
| `ZFC/Algebra.lean` | barrel (paraguas) de la fase | ✅ Activo |
| `Algebra/Monomial.lean` | Monomio: nulo `∅` (centinela), no nulo `⟨n,q⟩`; `monomMk`, `monomEval`, `monomDeg` (grado WithBot ω) | ✅ Completo (24 exports) |
| `Algebra/Polynomial.lean` | IsPolyQ, polyEval, grado, coeficiente líder | 📋 Planificado |
| `Algebra/PolyArith.lean` | suma/producto/negación; ℚ[X] dominio de integridad | 📋 Planificado |
| `Algebra/PolyDiv.lean` | división euclídea, polyGcd, TFA en ℚ[X] | 📋 Planificado |
...
└── ZFC/
    ...
    ├── Rat/                    # ℚ + Pow + Sucesiones + Cauchy + SqrtApprox/SqrtIrrational + TupleSeq (18 módulos)
    └── Algebra/                # Phase 8: Monomial.lean (monomios ℚ); Polynomial/PolyArith/PolyDiv pendientes
```

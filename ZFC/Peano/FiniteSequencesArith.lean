...
import ZFC.Nat.Mul
import ZFC.Peano.FiniteSequences
...
namespace ZFC
  ...
  namespace Peano.FiniteSequencesArith
    ...
    theorem mem_sumStepFn {f p : U} :
      ...
      unfold sumStepFn; rw [mem_sep_iff]
    ...
    theorem sumStepFn_is_function {f n : U} (hf : isFinSeq f n ω) :
        IsFunction (sumStepFn f) (ω ×ₛ ω) ω := by
      constructor
    ...
    theorem mem_familyProduct {F n g : U} :
        g ∈ familyProduct F n ↔
        (g ∈ FinSeqSet n (⋃ (image F n)) ∧ ∀ i, i ∈ n → g⦅i⦆ ∈ F⦅i⦆) := by
      unfold familyProduct; rw [mem_sep_iff]
      ...
      · -- (g ↾ n) ∈ familyProduct F n
        ...
        · -- g ↾ n ∈ FinSeqSet n (⋃ (image F n))
          ...
          have h_iy_in_g : ⟨i, y⟩ ∈ g :=
            ((mem_restrict_iff g n ⟨i, y⟩).mp hp).1
          have h_y_eq : y = g⦅i⦆ :=
            (apply_eq g i y (hg_fs.2.2 i (mem_succ_of_mem i n hfst_n)) h_iy_in_g).symm
          rw [h_y_eq]
          rw [mem_sUnion_iff]
          exact ⟨F⦅i⦆,
            (mem_range (F ↾ n) (F⦅i⦆)).mpr
              ⟨i, (mem_restrict_iff F n ⟨i, F⦅i⦆⟩).mpr
                ⟨isFinSeq_pair_mem hF (mem_succ_of_mem i n hfst_n),
                 by rw [fst_of_ordered_pair]; exact hfst_n⟩⟩,
            hg_choice i (mem_succ_of_mem i n hfst_n)⟩
        · -- ∀ i ∈ n, (g ↾ n)⦅i⦆ ∈ F⦅i⦆
          intro i hi
          rw [restrict_apply g n i hi]
          exact hg_choice i (mem_succ_of_mem i n hi)
      · -- g⦅n⦆ ∈ F⦅n⦆
        exact hg_choice n (mem_succ_self n)
      ...
      · constructor
        ...
        · intro x hx
          ...
          | inl hx_A =>
            ...
            exact ⟨y,
              (mem_union_iff f {⟨a, n⟩} ⟨x, y⟩).mpr (Or.inl hy_in),
              fun y' (hy' : ⟨x, y'⟩ ∈ union f {⟨a, n⟩}) => by
          ...
          | inr hx_a =>
            ...
            exact ⟨n,
              by rw [hx_a]; exact (mem_union_iff f {⟨a, n⟩} ⟨a, n⟩).mpr
    ...
    private theorem bijection_remove_top {B g m : U} (hm : m ∈ (ω : U))
      ...
      · constructor
        · intro p hp
          ...
          cases (mem_succ_iff m (snd p)).mp h_cp.2.2 with
          | inl h => exact h
          | inr h =>
            exfalso
        ...
        · intro x hx
          ...
          exact ⟨y,
            (mem_restrict_iff g (B ∖ {b₀}) ⟨x, y⟩).mpr
              ⟨hy, by rw [fst_of_ordered_pair]; exact hx⟩,
            fun y' hy' => hy_u y' (restrict_subset g (B ∖ {b₀}) ⟨x, y'⟩ hy')⟩
      · intro y hy
        have hy_sm := (mem_succ_iff m y).mpr (Or.inl hy)
        ...
        have hx_neq : x ≠ b₀ := by
          ...
          exact not_mem_self m hm_nat hy
        exact ⟨x, (mem_restrict_iff g (B ∖ {b₀}) ⟨x, y⟩).mpr
    ...
    private theorem product_singleton_equip (A b : U) :
      ...
      refine ⟨h, ?_, ?_, ?_⟩
      · constructor
        · intro p hp; exact ((mem_sep_iff _ p _).mp hp).1
        · intro x hx
          ...
          · exact (mem_sep_iff _ _ _).mpr
              ⟨(OrderedPair_mem_CartesianProduct x ⟨x, b⟩ A (A ×ₛ {b})).mpr
               ...
               x, hx, rfl⟩
          · obtain ⟨_, a, _, heq⟩ := (mem_sep_iff _ _ _).mp hy
            have p := Eq_of_OrderedPairs_given_projections x y a ⟨a, b⟩ heq
            rw [p.1]; exact p.2
      · intro x₁ x₂ y h₁ h₂
        obtain ⟨_, a₁, _, h₁_eq⟩ := (mem_sep_iff _ _ _).mp h₁
        obtain ⟨_, a₂, _, h₂_eq⟩ := (mem_sep_iff _ _ _).mp h₂
      ...
      · intro y hy
        ...
        refine ⟨fst y, (mem_sep_iff _ _ _).mpr ⟨?_, fst y, h_fst_A, ?_⟩⟩
    ...
    private theorem disjoint_union_equip {C₁ C₂ k n : U}
      ...
      · rw [mem_sep_iff]
        ...
        have h_eq : (C₁' ∪ C₂' : U) = C₁' := by
          apply ExtSet; intro z; rw [mem_union_iff]
      ...
      · intro n' hn'
        ...
        obtain ⟨c₀, hc₀⟩ := hg_surj n' (mem_succ_self n')
        have hc₀_C₂ : c₀ ∈ C₂' :=
        ...
        have h_decomp : C₂' = union (C₂' ∖ {c₀}) {c₀} := by
          apply ExtSet; intro z
          rw [mem_union_iff, SetDiff_is_specified, Singleton_is_specified]
          exact ⟨fun hz => if h : z = c₀ then Or.inr h
                            else Or.inl ⟨hz, h⟩,
                 fun h => h.elim (·.1) (by intro heq; rw [heq]; exact hc₀_C₂)⟩
        have h_union_eq : (C₁' ∪ C₂' : U) = union (C₁' ∪ (C₂' ∖ {c₀}) : U) {c₀} :=
          (congrArg (union C₁') h_decomp).trans
            (union_assoc C₁' (C₂' ∖ {c₀}) {c₀}).symm
        have hc₀_not : c₀ ∉ (C₁' ∪ (C₂' ∖ {c₀}) : U) := by
          intro h; rw [mem_union_iff] at h
          cases h with
          | inl h => exact (disjoint_comm C₁' C₂' hdisj') c₀ hc₀_C₂ h
    ...
    private theorem restriction_idempotent (f C : U) : (f ↾ C) ↾ C = f ↾ C := by
      ...
      constructor
      · intro hp
        exact ((mem_restrict_iff (f ↾ C) C p).mp hp).1
      · intro hp
        rw [mem_restrict_iff]
        exact ⟨hp, ((mem_restrict_iff f C p).mp hp).2⟩

    /-- image commutes with restriction idempotence:
        image (F ↾ n) n = image F n. -/
    private theorem image_restrict_eq (F n : U) :
        image (F ↾ n) n = image F n := by
      unfold image
    ...
    private theorem familyProduct_restriction (F n : U) :
      ...
      · intro h
        ...
        rw [image_restrict_eq] at h
        exact ⟨h.1, fun i hi => by rw [← restrict_apply F n i hi]; exact h.2 i hi⟩
      · intro h
        rw [mem_familyProduct] at h ⊢
        rw [image_restrict_eq]
        exact ⟨h.1, fun i hi => by rw [restrict_apply F n i hi]; exact h.2 i hi⟩
    ...
    /-- restrict nesting: if A ⊆ B then (f ↾ B) ↾ A = f ↾ A. -/
    private theorem restriction_nesting {f A B : U} (hAB : A ⊆ B) :
        (f ↾ B) ↾ A = f ↾ A := by
      apply ExtSet; intro p
      constructor
      · intro hp
        have h := (mem_restrict_iff (f ↾ B) A p).mp hp
        have hp_fB := (mem_restrict_iff f B p).mp h.1
        exact (mem_restrict_iff f A p).mpr ⟨hp_fB.1, h.2⟩
      · intro hp
        have h := (mem_restrict_iff f A p).mp hp
        exact (mem_restrict_iff (f ↾ B) A p).mpr
          ⟨(mem_restrict_iff f B p).mpr ⟨h.1, hAB _ h.2⟩, h.2⟩
    ...
    private theorem isFinSeq_restriction_general {f n A C : U}
        ...
        isFinSeq (f ↾ C) C A :=
      ⟨hC_omega, restrict_is_function f n A C h.2 hC⟩
    ...
    private theorem seqProd_restriction {c n : U}
      ...
      suffices hP : P n from
        hP c (σ n) hc (fun x hx => mem_succ_of_mem x n hx)
      let S := sep (ω : U) P
      suffices hS : S = ω by
        have hn_S : n ∈ S := hS ▸ hn
        exact ((mem_sep_iff (ω : U) n P).mp hn_S).2
      ...
      · -- Step: P(k) → P(σ k)
        ...
        have hk_sub : k ⊆ n' := fun x hx =>
          hsk_sub x (mem_succ_of_mem x k hx)
        ...
        rw [restrict_apply c' (σ k) k (mem_succ_self k)]
        ...
        have h_nest : (c' ↾ σ k) ↾ k = c' ↾ k :=
          restriction_nesting (fun x hx => mem_succ_of_mem x k hx)
        have h1 := ih (c' ↾ σ k) (σ k) hcr_sk
          (fun x hx => mem_succ_of_mem x k hx)
    ...
    private theorem familyProduct_succ_decomp {F' n' A' : U} (hn' : n' ∈ (ω : U))
      ...
      -- image monotonicity
      have h_img_sub : ⋃ (image F' n') ⊆ ⋃ (image F' (σ n')) := by
        intro x hx
        rw [mem_sUnion_iff] at hx ⊢
        obtain ⟨S, hS_img, hx_S⟩ := hx
        refine ⟨S, ?_, hx_S⟩
        unfold image at hS_img ⊢
        rw [mem_range] at hS_img ⊢
        obtain ⟨i, hi⟩ := hS_img
        have hp := (mem_restrict_iff F' n' ⟨i, S⟩).mp hi
        refine ⟨i, (mem_restrict_iff F' (σ n') ⟨i, S⟩).mpr ⟨hp.1, ?_⟩⟩
        rw [fst_of_ordered_pair] at hp ⊢
        exact mem_succ_of_mem i n' hp.2
      -- Forward decomposition
      have h_fwd : ∀ g, g ∈ familyProduct F' (σ n') →
          (g ↾ n') ∈ familyProduct F' n' ∧ g⦅n'⦆ ∈ F'⦅n'⦆ := by
        intro g hg
        have hg_mp := mem_familyProduct.mp hg
        have hg_fs : isFinSeq g (σ n') (⋃ image F' (σ n')) :=
        ...
        · rw [mem_familyProduct]
          ...
          · intro p hp
            ...
            rw [fst_of_ordered_pair] at hp'
            rw [snd_of_ordered_pair]
            have h_iy_in_g := ((mem_restrict_iff g n' ⟨i₀, y₀⟩).mp hp).1
            have h_y_eq : y₀ = g⦅i₀⦆ :=
              (apply_eq g i₀ y₀ (hg_fs.2.2 i₀
                (mem_succ_of_mem i₀ n' hp'.2.1)) h_iy_in_g).symm
            rw [h_y_eq, mem_sUnion_iff]
            exact ⟨F'⦅i₀⦆,
              (mem_range (F' ↾ n') (F'⦅i₀⦆)).mpr ⟨i₀,
                (mem_restrict_iff F' n' ⟨i₀, F'⦅i₀⦆⟩).mpr
                  ⟨isFinSeq_pair_mem hF' (mem_succ_of_mem i₀ n' hp'.2.1),
                   by rw [fst_of_ordered_pair]; exact hp'.2.1⟩⟩,
              hg_choice i₀ (mem_succ_of_mem i₀ n' hp'.2.1)⟩
          · rw [restrict_apply g n' i hi]
            exact hg_choice i (mem_succ_of_mem i n' hi)
        · exact hg_choice n' (mem_succ_self n')
      ...
      let Fn' := F'⦅n'⦆
      let φ := sep (FPsn ×ₛ (FPn ×ₛ Fn'))
        (fun p => ∃ g, g ∈ FPsn ∧ p = ⟨g, ⟨g ↾ n', g⦅n'⦆⟩⟩)
      refine ⟨φ, ?_, ?_, ?_⟩
      · -- IsFunction
        constructor
        · exact fun p hp => ((mem_sep_iff _ p _).mp hp).1
        · intro x hx
          have hsc := h_fwd x hx
          ...
          · exact (mem_sep_iff _ _ _).mpr
              ⟨(OrderedPair_mem_CartesianProduct x ⟨x ↾ n', x⦅n'⦆⟩ FPsn (FPn ×ₛ Fn')).mpr
                ⟨hx, (OrderedPair_mem_CartesianProduct (x ↾ n') (x⦅n'⦆) FPn Fn').mpr
                  ⟨hsc.1, hsc.2⟩⟩,
               x, hx, rfl⟩
          · obtain ⟨_, g, hg_mem, heq⟩ := (mem_sep_iff _ _ _).mp hy
            have hpair := Eq_of_OrderedPairs_given_projections x y g ⟨g ↾ n', g⦅n'⦆⟩ heq
            rw [hpair.1]; exact hpair.2
      · -- isInjective
        intro x₁ x₂ y h₁ h₂
        obtain ⟨_, g₁, hg₁_mem, heq₁⟩ := (mem_sep_iff _ _ _).mp h₁
        obtain ⟨_, g₂, hg₂_mem, heq₂⟩ := (mem_sep_iff _ _ _).mp h₂
        ...
        have hg₁_fs : isFinSeq g₁ (σ n') (⋃ image F' (σ n')) :=
          mem_FinSeqSet_iff.mp ((mem_familyProduct.mp hg₁_mem).1)
        have hg₂_fs : isFinSeq g₂ (σ n') (⋃ image F' (σ n')) :=
          mem_FinSeqSet_iff.mp ((mem_familyProduct.mp hg₂_mem).1)
        have h_ext : g₁ = g₂ := isFinSeq_ext hg₁_fs hg₂_fs (fun i hi => by
          cases (mem_succ_iff n' i).mp hi with
          | inl h_in =>
            rw [← restrict_apply g₁ n' i h_in, ← restrict_apply g₂ n' i h_in,
                h_pair.1]
      ...
      · -- isSurjectiveOnto
        ...
        have hfs : isFinSeq (fst y) n' (⋃ image F' n') :=
        ...
        have h_widen : isFinSeq (fst y) n' (⋃ image F' (σ n')) :=
          ⟨hfs.1, fun p hp =>
            CartesianProduct_mono n' n' _ _ (fun x hx => hx) h_img_sub _ (hfs.2.1 p hp),
           hfs.2.2⟩
        have h_snd_union : snd y ∈ ⋃ (image F' (σ n')) := by
          rw [mem_sUnion_iff]
          refine ⟨F'⦅n'⦆, ?_, hsnd_F⟩
          unfold image; rw [mem_range]
          exact ⟨n', (mem_restrict_iff F' (σ n') ⟨n', F'⦅n'⦆⟩).mpr
            ⟨isFinSeq_pair_mem hF' (mem_succ_self n'),
             by rw [fst_of_ordered_pair]; exact mem_succ_self n'⟩⟩
        ...
        have h_restr_eq : appendElem (fst y) n' (snd y) ↾ n' = fst y :=
          isFinSeq_ext (isFinSeq_restriction hg_fs) h_widen (fun i hi => by
            rw [restrict_apply (appendElem (fst y) n' (snd y)) n' i hi,
    ...
    theorem card_familyProduct {F c n : U}
      ...
      · -- BASE: P(∅)
        ...
        have h_succ_zero : σ (∅ : U) = {∅} := empty_union {∅}
        rw [h_succ_zero]
        exact equipotent_refl {∅}
      · -- STEP: P(n') → P(σ n')
        intro n' hn'
        ...
        have hcard_r : ∀ i, i ∈ n' → (F' ↾ n')⦅i⦆ ≃ₛ (c' ↾ n')⦅i⦆ := fun i hi => by
          rw [restrict_apply F' n' i hi, restrict_apply c' n' i hi]
          exact hcard' i (mem_succ_of_mem i n' hi)
        ...
        exact equipotent_trans h_decomp
          (card_product_two hsp_w hcn'_w ih_result (hcard' n' (mem_succ_self n')))
